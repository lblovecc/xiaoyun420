...
(function() {
    ...
    var LuShu =
         ...
         */
        BMapLib.LuShu = function(track, map, path, opts) {
            if (!path || path.length < 1) {
                return;
            }
            this._trackControl = track;
            this._map = map;
            //存储一条路线
            this._path = path;
            //移动到当前点的索引
    ...
    LuShu.prototype.start = function() {
        ...
        if (me.i && me.i < len - 1) {
            ...
            if (me._trackControl._timeOverlay) {
                me._map.removeOverlay(me._trackControl._timeOverlay);
            }
            me._addMarker();
            me._trackControl.lushuMarker = me._marker;
            me._trackControl.lushuMarker.trackId = me._trackControl.currentPlayTrack;
    ...
    baidu.object.extend(LuShu.prototype, {
        ...
        _addInfoWin: function() {
            ...
            this._trackControl._timeOverlay = overlay;
        ...
        _move: function(initPos, targetPos, effect) {
            ...
            init_pos.loc_time = initPos.loc_time;
            target_pos.loc_time = targetPos.loc_time;
            //如果小于1直接移动到下一点
            if (count < 1) {
                me._moveNext(++me.i);
                return;
            ...
            me._intervalFlag = setInterval(function() {
                ...
                if (currentCount >= count) {
                    ...
                    pos.loc_time = target_pos.loc_time;
                    //设置marker
                    if (currentCount == 1) {
                        var proPos = null;
                        if (me.i - 1 >= 0) {
                    ...
                    //设置自定义overlay的位置
                    var track_id = me._trackControl.currentPlayTrack;
                    if (!me._trackControl.timeControl.isVisible()) {
                        me._trackControl.timeControl.show();
                    }
                    me._trackControl.setTimeControl(pos.loc_time);
                    me._trackControl.setSliderTime(pos.loc_time);
                    //me._setInfoWin(pos, pos.loc_time);
                }
            }, timer);
        },
        /**
        ...
        setRotation: function(prePos, curPos, targetPos) {
            ...
            curPos = this._projection.lngLatToPoint(curPos);
         ...
         * @return 无返回值.
         */
        _setInfoWin: function(pos, content) {
